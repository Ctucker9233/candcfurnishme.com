...
#[AsCommand(
        name: 'app:package-cache',
        description: 'Cache Package Data'   
    )]
class PackageCacheCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        try
        {
            ...
            $value = $this->entityManager->getRepository(Packages::class)->findAll();
            $result = $this->packageHelper->dbPackageProcessor($value);
            //dump($result);
            $packageCache->set([$result]);
            $cache->save($packageCache);
