...
#[AsCommand(
        name: 'app:yourco-update',
        description: 'Update items and locations'   
    )]
class YourcoUpdateCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        catch(\Exception $exception)
        {
            ...
            $task = [
               'name' => "Yourco update",
               'command' => "app:yourco-update",
               'status' => "success",
               'startedAt' => $date,
               'message' => $error
            ];

            $taskLog = new JsonResponse($task);

            $job = $this->serializer->deserialize($taskLog->getContent(), Tasks::class, 'json');
