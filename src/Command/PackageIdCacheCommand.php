...
#[AsCommand(
        name: 'app:packageId-cache',
        description: 'Cache Package ID Data'   
    )]
class PackageIdCacheCommand extends Command
{
    ...
    {
        $this->addArgument('', InputArgument::OPTIONAL, 'vendor');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        try
        {
            ...
            $packageIdCache = $cache->getItem('packageIds');
            $value = $this->entityManager->getRepository(Inventory::class)->findAll();
            $result = $this->packageHelper->dbPackageIdProcessor($value);
            //dump($result);
            $packageIdCache->set([$result]);
            $cache->save($packageIdCache);
